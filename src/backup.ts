 ...
 */
export function createBackup(filePath: string, strategy: BackupStrategy): BackupResult {
  try {
    switch (strategy) {
      case "copy": {
      ...
      default:
        throw new BackupError(filePath, `Unknown backup strategy: ${strategy as string}`)
    }
  } catch (err) {
    throw new BackupError(filePath, err instanceof Error ? err.message : String(err))
  }
}
 ...
 */
export function restoreBackup(backup: BackupResult, originalPath: string): void {
  try {
