...
describe("findMediaFiles", () => {
  ...
  it("respects recursive flag", async () => {
    ...
    const nonRecursive = findMediaFiles(join(testDir, "parent"), [".mp4"], false)
...
describe("integration", () => {
  ...
  itIf("measures loudness of an audio file", async () => {
    const { detectFfmpeg, measureLoudness } = await import("../src/ffmpeg/index.ts")
    const ffmpegPath = detectFfmpeg()
    ...
    const measurement = await measureLoudness(ffmpegPath, inputFile, -14, 7, -2, 2)
  ...
  itIf("normalizes a short audio file", async () => {
    ...
    const measurement = await measureLoudness(ffmpegPath, inputFile, -14, 7, -2, 1)
    expect(measurement).not.toBeNull()

    const m = measurement as NonNullable<typeof measurement>

    await normalizeMediaFile(ffmpegPath, inputFile, outputFile, m, opts, 1, undefined, undefined)

    expect(existsSync(outputFile)).toBe(true)

    // Verify the output is playable
    const probe = await (await import("../src/ffmpeg/index.ts")).probeMedia(outputFile, ffmpegPath)
    expect(probe.hasAudio).toBe(true)
  })
})
